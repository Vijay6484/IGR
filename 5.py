...
def run_scraper_for_year(year, window_position):
    ...
    def process_indexii_buttons(driver, wait, initial_buttons, meta, page_label, suffix):
        ...
        seen_docs = meta.get('seen_docs_set', set())
        ...
        for idx in range(1, total_buttons + 1):
            try:
                ...
                try:
                    ...
                    if onclick_val:
                        row_id = hashlib.md5(onclick_val.encode('utf-8')).hexdigest()
                    else:
                        row_elem = button.find_element(By.XPATH, "./ancestor::tr")
                        row_text = row_elem.text.strip()
                        row_id = hashlib.md5(row_text.encode('utf-8')).hexdigest()
                        
                    if row_id in seen_docs:
                        safe_print(f"[DEDUPLICATE] Skipping already downloaded document at row {idx}")
                        continue
                except Exception as e:
                    row_id = None
                    safe_print(f"[DEDUPLICATE WARN] Could not generate unique ID: {e}")
            ...
            for t_name, t_val in tahsil_options:
                ...
                for v_name, v_val in village_options:
                    ...
                    for gut_no in range(max(1, int(last_gut) + 1), 21):
                        ...
                        # Refresh website and re-enter details for next property
                        if gut_no < 20:
                            safe_print(f"[GUT {gut_no}] Refreshing website and re-entering details for next property...")
                            if not safe_get_url(driver, WEBSITE_URL):
                                safe_print("[ERROR] Failed to refresh website after property")
                                break
                            driver, wait = close_popup_and_click_rest(driver, wait)
                            Select(driver.find_element(By.ID, "ddlFromYear1")).select_by_visible_text(year)
